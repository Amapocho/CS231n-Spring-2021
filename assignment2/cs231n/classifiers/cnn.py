...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dl_dx, grads['W3'], grads['b3'] = affine_backward(g_softmax, clas_cache)
        grads['W3'] += self.reg * W3
        loss += np.sum(np.power(W3, 2))

        dl_dx, grads['W2'], grads['b2'] = affine_relu_backward(dl_dx, hidn_cache)
        grads['W2'] += self.reg * W2
        loss += np.sum(np.power(W2, 2))

        dl_dx, grads['W1'], grads['b1'] = conv_relu_pool_backward(dl_dx, conv_cache)
        grads['W1'] += self.reg * W1
        loss += np.sum(np.power(W1, 2))
