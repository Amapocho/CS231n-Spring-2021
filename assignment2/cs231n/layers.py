...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = x, x_hat, mu, var, eps, gamma
...
def batchnorm_backward(dout, cache):
    ...
    N, D = dout.shape
    x, x_hat, mu, var, eps, gamma = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(x_hat * dout, axis=0)
    dvar = np.sum(gamma * dout * (x - mu) * (-0.5) * np.power(var + eps, -1.5), axis=0)
    dmean = np.sum(gamma * dout * (-1.0) * np.power(var + eps, -0.5), axis=0) + dvar * (-2.0 / N) * np.sum(x - mu, axis=0)
    dx = dout * gamma * np.power(var + eps, -0.5) + dvar * 2.0 * (x - mu) / N + dmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(x_hat * dout, axis=0)
    dxhat = dout * gamma
    dx = (1. / (N * np.sqrt(var + eps))) * (N*dxhat - np.sum(dxhat, axis=0) - x_hat*np.sum(dxhat*x_hat, axis=0))
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    mu = np.mean(x, axis=1)
    var = np.var(x, axis=1)
    x_hat = ((x.T - mu) / (np.sqrt(var + eps))).T
    out = gamma * x_hat + beta
...
def layernorm_backward(dout, cache):
    ...
    dxhat = dout * gamma
    dx = ((1. / (D * np.sqrt(var + eps))) * (D * dxhat.T - np.sum(dxhat, axis=1) - x_hat.T*np.sum(dxhat*x_hat, axis=1))).T
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = np.zeros((N, F, 1 + (H_pad - HH) // stride, 1 + (W_pad - WW) // stride))
    for n in range(N):
      for f in range(F):
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for i in range(0, H - pool_height + 1, stride):
          for j in range(0, W - pool_width + 1, stride):
            index = np.unravel_index(np.argmax(x[n, c, i : i + pool_height, j : j + pool_width]), (pool_height, pool_width))
            dx[n, c, index[0] + i, index[1] + j] = dout[n, c, i//stride, j//stride]
