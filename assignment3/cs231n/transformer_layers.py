...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        exp_term = 10000**(-torch.arange(0, embed_dim, 2)/embed_dim)
        pe[0, :, 0::2] = torch.sin(position * exp_term)
        pe[0, :, 1::2] = torch.cos(position * exp_term) 
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.H
        d = D // H
        query = self.query(query).reshape((N, S, H, d)).transpose(1, 2)
        value = self.value(value).reshape((N, T, H, d)).transpose(1, 2)
        key = self.key(key).reshape((N, T, H, d)).transpose(1, 2)
        scores = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(d)
        if attn_mask is not None:
          attn_mask = attn_mask.bool()
          scores= scores.masked_fill_(attn_mask == 0, -float('inf'))

        weights = self.dropout(self.softmax(scores))
        output = torch.matmul(weights, value).transpose(1, 2).reshape((N, S, -1))
