...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += -((sim(z_k, z_k_N) / tau).exp() / (sum(map(lambda x: (sim(z_k, x) / tau).exp(), out)) - (sim(z_k, z_k) / tau).exp())).log()
        total_loss += -((sim(z_k_N, z_k) / tau).exp() / (sum(map(lambda x: (sim(z_k_N, x) / tau).exp(), out)) - (sim(z_k_N, z_k_N) / tau).exp())).log()
...
def compute_sim_matrix(out):
    ...
    sim_matrix = out @ out.T / torch.outer(norm, norm)
