...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, temp = cache

    dupper = dnext_h * (1 - (np.tanh(temp)**2))
    dx = np.dot(dupper,Wx.T)
    dprev_h = np.dot(dupper,Wh.T)
    dWx = np.dot(x.T,dupper)
    dWh = np.dot(prev_h.T,dupper)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      prev_h, grad_cache[i] = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
      h[:, i, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dx_t, dnext_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, i, :] + dnext_h, grad_cache[i])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
        dx[:, i, :] = dx_t
